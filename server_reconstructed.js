...
app.post('/api/purge-audio', (req, res) => {
    try {
        ...
        if (fs.existsSync(resultFrPath)) {
            fs.unlinkSync(resultFrPath);
        }
        enforceMax10RollingRetention();
...
app.post('/api/upload-and-translate', upload.single('file'), async (req, res) => {
    try {
        ...
        const filename = req.file.filename;
        const targetLang = req.body.target_lang || 'fr'; // 'fr' pour traduire vers le Français, 'ar' pour traduire vers l'Arabe
        ...
        if (targetLang === 'fr') {
            ...
            // Traduction du Français vers l'Arabe Palestinien
            await runPython(`py process_single_file.py "${filePath}"`);
            const outPath = path.join(__dirname, 'single_process_out.json');
...
app.post('/api/chat/send-audio', upload.single('audio'), async (req, res) => {
    try {
        ...
        await runPython(`py process_single_file.py "${audioFilePath}"`);
...
app.listen(PORT, () => {
    console.log(`Server running on http://localhost:3000 (Chat Éphémère 24h & Sélection Windows Explorer & WhatsApp activé)`);
});
